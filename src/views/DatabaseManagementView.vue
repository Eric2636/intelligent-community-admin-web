...
function confirmManualBackup() { Modal.confirm({ title: '立即执行数据库备份？', content: '任务成功并完整校验后，会覆盖当前环境上一份有效备份文件。', okText: '开始备份', cancelText: '取消', async onOk() { creatingBackup.value = true; try { await createDatabaseBackup(); message.success('备份任务已创建'); jobPagination.current = 1; await Promise.all([loadJobs(), loadOverview()]); } catch (error) { message.error(errorMessage(error)); throw error; } finally { creatingBackup.value = false; } } }); }
